...
def list_cards(session: Session, arguments: List[str]):
    ...
    columns = {column_id: (column_name, [])
               for column_id, column_name in column_names}
    query = Query(session,
                  '''select cards.id,
                  cards.name,
                  columns.id
    ...
    with query as cursor:
        ...
        for card_id, card_name, column_id in cursor:
            columns[column_id][1].append((card_id, card_name))
    for column_id, (column_name, card_names) in columns.items():
